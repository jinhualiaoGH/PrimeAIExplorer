...
# Step 4 - Canonical Prompt Specification
# ============================================================
...
$CanonicalPromptPath = Join-Path $ArchitectureDir "Canonical_Prompt.md"
$RegistryCsvPath = Join-Path $PromptsDir "prompt_registry.csv"
$RegistryJsonPath = Join-Path $PromptsDir "prompt_registry.json"
...
$RegistryRows = @(
    ...
    [pscustomobject][ordered]@{
        prompt_id           = "PROMPT-000002"
        title               = "Prime Gap Compression Prompt"
        short_name          = "prime_gap_compression"
        version             = "0.1.0"
        status              = "Proposed"
        prompt_family       = "compression"
        experiment_id       = "EXP-000002"
        dataset_id          = "DS-000002"
        response_schema_id  = "RESPONSE-000002"
        created_date        = "2026-07-25"
        modified_date       = "2026-07-25"
    },
    [pscustomobject][ordered]@{
        prompt_id           = "PROMPT-000003"
        title               = "Prime Structure Abstraction Prompt"
        short_name          = "prime_structure_abstraction"
        version             = "0.1.0"
        status              = "Proposed"
        prompt_family       = "abstraction"
        experiment_id       = "EXP-000003"
        dataset_id          = "DS-000003"
        response_schema_id  = "RESPONSE-000003"
        created_date        = "2026-07-25"
        modified_date       = "2026-07-25"
    }
)
...
$RegistryObject = [ordered]@{
    registry_name    = "PrimeAIExplorer Prompt Registry"
...
Write-Host ""
Write-Host "============================================================"
Write-Host " PrimeAIExplorer v0.2 - Step 4"
Write-Host " Canonical Prompt Specification"
Write-Host "============================================================"
Write-Host ""
...
$DocumentContent = Get-Content $CanonicalPromptPath -Raw
...
try {
    ...
    if ($RegistryJson.prompts.Count -eq 3) {
        Write-Host "[PASS] Registry JSON contains 3 prompts"
    }
    else {
        Write-Host "[FAIL] Registry JSON prompt count is not 3"
        $Failed = $true
    }
}
catch {
    Write-Host "[FAIL] Prompt registry JSON is invalid"
    Write-Host $_.Exception.Message
    $Failed = $true
}

try {
    $CsvRows = Import-Csv $RegistryCsvPath

    if ($CsvRows.Count -eq 3) {
        Write-Host "[PASS] Registry CSV contains 3 prompts"
    }
    else {
        Write-Host "[FAIL] Registry CSV prompt count is not 3"
        $Failed = $true
    }
}
catch {
    Write-Host "[FAIL] Prompt registry CSV could not be read"
    Write-Host $_.Exception.Message
    $Failed = $true
}

$CsvIds = @(
    (Import-Csv $RegistryCsvPath).prompt_id |
        Sort-Object
)

$JsonIds = @(
    $RegistryJson.prompts.prompt_id |
        Sort-Object
)

if (($CsvIds -join "|") -eq ($JsonIds -join "|")) {
    Write-Host "[PASS] CSV and JSON prompt identifiers agree"
}
else {
    Write-Host "[FAIL] CSV and JSON prompt identifiers differ"
    $Failed = $true
}

$DuplicateIds = @(
    $RegistryRows |
        Group-Object prompt_id |
        Where-Object Count -gt 1
)

if ($DuplicateIds.Count -eq 0) {
    Write-Host "[PASS] No duplicate prompt identifiers"
}
else {
    Write-Host "[FAIL] Duplicate prompt identifiers detected"
    $Failed = $true
}
...
$InvalidIds = @(
    $RegistryRows |
        Where-Object {
            $_.prompt_id -notmatch "^PROMPT-[0-9]{6}$"
        }
)

if ($InvalidIds.Count -eq 0) {
    Write-Host "[PASS] All prompt identifiers are canonical"
}
else {
    Write-Host "[FAIL] Invalid prompt identifier format"
    $InvalidIds | Format-Table -AutoSize
    $Failed = $true
}
...
$InvalidVersions = @(
    $RegistryRows |
        Where-Object {
            $_.version -notmatch "^[0-9]+\.[0-9]+\.[0-9]+$"
        }
)

if ($InvalidVersions.Count -eq 0) {
    Write-Host "[PASS] All prompt versions use semantic versioning"
}
else {
    Write-Host "[FAIL] Invalid prompt version format"
    $InvalidVersions | Format-Table -AutoSize
    $Failed = $true
}

$ExperimentRegistryPath = Join-Path $Root "experiments\experiment_registry.csv"
$DatasetRegistryPath = Join-Path $Root "datasets\dataset_registry.csv"

if (Test-Path $ExperimentRegistryPath) {
    $ExperimentIds = @(
        (Import-Csv $ExperimentRegistryPath).experiment_id
    )

    $MissingExperiments = @(
        $RegistryRows |
            Where-Object {
                $_.experiment_id -notin $ExperimentIds
            }
    )

    if ($MissingExperiments.Count -eq 0) {
        Write-Host "[PASS] All prompts reference registered experiments"
    }
    else {
        Write-Host "[FAIL] Prompt references unknown experiment"
        $MissingExperiments | Format-Table -AutoSize
        $Failed = $true
    }
}
else {
    Write-Host "[FAIL] Experiment registry is missing"
    $Failed = $true
}

if (Test-Path $DatasetRegistryPath) {
    $DatasetIds = @(
        (Import-Csv $DatasetRegistryPath).dataset_id
    )

    $MissingDatasets = @(
        $RegistryRows |
            Where-Object {
                $_.dataset_id -notin $DatasetIds
            }
    )

    if ($MissingDatasets.Count -eq 0) {
        Write-Host "[PASS] All prompts reference registered datasets"
    }
    else {
        Write-Host "[FAIL] Prompt references unknown dataset"
        $MissingDatasets | Format-Table -AutoSize
        $Failed = $true
    }
}
else {
    Write-Host "[FAIL] Dataset registry is missing"
    $Failed = $true
}

Write-Host ""
Write-Host "Prompt registry:"
...
Write-Host ""
Write-Host "Canonical document line count:"

$LineCount = (Get-Content $CanonicalPromptPath).Count
Write-Host $LineCount

if ($LineCount -lt 300) {
    Write-Host "[WARN] Canonical prompt document is shorter than expected"
}
