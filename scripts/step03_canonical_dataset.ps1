...
# Step 3 - Canonical Dataset Specification
# ============================================================
...
$CanonicalDatasetPath = Join-Path $ArchitectureDir "Canonical_Dataset.md"
$RegistryCsvPath = Join-Path $DatasetsDir "dataset_registry.csv"
$RegistryJsonPath = Join-Path $DatasetsDir "dataset_registry.json"
...
$RegistryRows = @(
    ...
    [pscustomobject][ordered]@{
        ...
        title               = "Prime Gap Compression Dataset"
        short_name          = "prime_gap_compression"
        version             = "0.1.0"
        status              = "Proposed"
        source_universe     = "PrimeNet"
        dataset_family      = "compression_tasks"
        primary_experiment  = "EXP-000002"
        created_date        = "2026-07-25"
        modified_date       = "2026-07-25"
    },
    [pscustomobject][ordered]@{
        dataset_id          = "DS-000003"
        title               = "Prime Structure Abstraction Dataset"
        short_name          = "prime_structure_abstraction"
        version             = "0.1.0"
        status              = "Proposed"
        source_universe     = "PrimeNet"
        dataset_family      = "abstraction_tasks"
        primary_experiment  = "EXP-000003"
        created_date        = "2026-07-25"
        modified_date       = "2026-07-25"
    }
)
...
$RegistryObject = [ordered]@{
    registry_name    = "PrimeAIExplorer Dataset Registry"
...
Write-Host ""
Write-Host "============================================================"
Write-Host " PrimeAIExplorer v0.2 - Step 3"
Write-Host " Canonical Dataset Specification"
Write-Host "============================================================"
Write-Host ""
...
$RequiredPhrases = @(
    ...
    "Dataset Versioning",
    "Leakage Prevention",
    "PrimeNet-Derived Dataset Requirements",
    "Prime Gap Memory-Limited Learning Dataset",
    "Datasets are not merely inputs to model calls.",
    "Make observations first.",
    "Draw conclusions second."
...
$DocumentContent = Get-Content $CanonicalDatasetPath -Raw
...
try {
    ...
    if ($RegistryJson.datasets.Count -eq 3) {
        Write-Host "[PASS] Registry JSON contains 3 datasets"
    }
    else {
        Write-Host "[FAIL] Registry JSON dataset count is not 3"
        $Failed = $true
    }
}
catch {
    Write-Host "[FAIL] Dataset registry JSON is invalid"
...
try {
    ...
    if ($CsvRows.Count -eq 3) {
        Write-Host "[PASS] Registry CSV contains 3 datasets"
    }
    else {
        Write-Host "[FAIL] Registry CSV dataset count is not 3"
        $Failed = $true
    }
}
catch {
    Write-Host "[FAIL] Dataset registry CSV could not be read"
    Write-Host $_.Exception.Message
    $Failed = $true
}

$CsvIds = @(
    (Import-Csv $RegistryCsvPath).dataset_id |
        Sort-Object
)

$JsonIds = @(
    $RegistryJson.datasets.dataset_id |
        Sort-Object
)

if (($CsvIds -join "|") -eq ($JsonIds -join "|")) {
    Write-Host "[PASS] CSV and JSON dataset identifiers agree"
}
else {
    Write-Host "[FAIL] CSV and JSON dataset identifiers differ"
    $Failed = $true
}

$DuplicateIds = @(
    $RegistryRows |
        Group-Object dataset_id |
        Where-Object Count -gt 1
)

if ($DuplicateIds.Count -eq 0) {
    Write-Host "[PASS] No duplicate dataset identifiers"
}
else {
    Write-Host "[FAIL] Duplicate dataset identifiers detected"
    $Failed = $true
}
...
$InvalidIds = @(
    $RegistryRows |
        Where-Object {
            $_.dataset_id -notmatch "^DS-[0-9]{6}$"
        }
)

if ($InvalidIds.Count -eq 0) {
    Write-Host "[PASS] All dataset identifiers are canonical"
}
else {
    Write-Host "[FAIL] Invalid dataset identifier format"
...
if ($InvalidVersions.Count -eq 0) {
    Write-Host "[PASS] All dataset versions use semantic versioning"
}
else {
    Write-Host "[FAIL] Invalid dataset version format"
    $InvalidVersions | Format-Table -AutoSize
    $Failed = $true
}

Write-Host ""
Write-Host "Dataset registry:"
...
$LineCount = (Get-Content $CanonicalDatasetPath).Count
Write-Host $LineCount

if ($LineCount -lt 300) {
    Write-Host "[WARN] Canonical dataset document is shorter than expected"
}
