...
def analyze(
    ...
    return {
        ...
        "ledger_entry_count": ledger_entries if ledger_entries is not None else len(records),
        "completed_entry_count": len(records),
        "pending_entry_count": pending_entries,
        "pilot_completion": (len(records) / ledger_entries) if ledger_entries else 0.0,
