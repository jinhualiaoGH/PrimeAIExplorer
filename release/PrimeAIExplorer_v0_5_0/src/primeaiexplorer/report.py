...
def html_report(summary, metadata):
    ...
    return f'''<!doctype html><html><head><meta charset="utf-8"><title>PrimeAIExplorer v0.5.0</title><style>
...
</style></head><body><header><h1>PrimeAIExplorer v0.5.0</h1><p>{html.escape(metadata['experiment_id'])} · {html.escape(metadata['pilot_id'])} · {html.escape(metadata['model'])}</p></header><main>
<div class="cards"><div class="card"><div>Completed</div><div class="big">{summary['record_count']} / {summary['ledger_entry_count']}</div></div><div class="card"><div>Accuracy</div><div class="big">{_pct(summary['accuracy'])}</div></div><div class="card"><div>Brier</div><div class="big">{summary['brier_score']:.4f}</div></div><div class="card"><div>ECE</div><div class="big">{summary['ece']:.4f}</div></div><div class="card"><div>Entropy</div><div class="big">{summary['prediction_entropy_bits']:.3f}</div></div><div class="card"><div>Distinct predictions</div><div class="big">{summary['distinct_predictions']}</div></div><div class="card"><div>Mean |error|</div><div class="big">{summary['mean_absolute_error']:.2f}</div></div><div class="card"><div>Signed error</div><div class="big">{summary['mean_signed_error']:+.2f}</div></div></div>
