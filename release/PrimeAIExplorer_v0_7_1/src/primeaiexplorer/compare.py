...
def _rank(rows: list[dict[str, Any]], key: str, *, lower_is_better: bool) -> None:
    ordered = sorted(
        rows,
        key=lambda row: (
            float(row.get(key, 0.0)) if row.get(key) is not None else float("inf"),
            str(row.get("label", "")),
        ),
...
def build_comparison(analyses: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    rankings = [
        {
            "metric": metric,
            "best_label": min(rows, key=lambda r: float(r.get(metric, 0.0)))["label"]
            if lower
            else max(rows, key=lambda r: float(r.get(metric, 0.0)))["label"],
            "best_value": min(float(r.get(metric, 0.0)) for r in rows)
            if lower
            else max(float(r.get(metric, 0.0)) for r in rows),
    ...
    return {
        ...
        "aggregate": {
            ...
            "mean_brier_score": mean(float(row.get("brier_score", 0.0)) for row in rows),
            "mean_ece": mean(float(row.get("ece", 0.0)) for row in rows),
            "mean_absolute_error": mean(float(row.get("mean_absolute_error", 0.0)) for row in rows),
        },
        "sources": [
            {
                "label": item["label"],
...
def markdown_report(comparison: dict[str, Any]) -> str:
    lines = [
        "# PrimeAIExplorer v0.7.1 Comparative Observatory",
        "",
    ...
    for row in comparison["comparison_rows"]:
        lines.append(
            f"| {row['label']} | {row.get('record_count', 0)} | {float(row.get('accuracy', 0)):.2%} | "
            f"{float(row.get('brier_score', 0)):.4f} | {float(row.get('ece', 0)):.4f} | "
            f"{float(row.get('mean_absolute_error', 0)):.2f} | {float(row.get('switch_rate', 0)):.2%} | "
            f"{row.get('favorite_prediction', '')} |"
        )
...
def html_report(comparison: dict[str, Any]) -> str:
    rows = "".join(
        ...
        f"<td>{float(row.get('brier_score', 0)):.4f}</td>"
        f"<td>{float(row.get('ece', 0)):.4f}</td>"
        f"<td>{float(row.get('prediction_entropy_bits', 0)):.3f}</td>"
        ...
        f"<td>{row.get('favorite_prediction', '')}</td>"
        "</tr>"
        for row in comparison["comparison_rows"]
    )
    ranking_cards = "".join(
    ...
    return f"""<!doctype html><html><head><meta charset='utf-8'><title>PrimeAIExplorer v0.7.1</title><style>
...
</style></head><body><header><h1>PrimeAIExplorer v0.7.1</h1><p>Comparative Observatory · {comparison['analysis_count']} analyses</p></header><main>
