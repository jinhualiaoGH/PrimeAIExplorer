...
class VisualizationDashboardTests(unittest.TestCase):
    ...
    def test_reliability_has_numeric_ticks(self):
        svg = SvgVisualizationEngine().render_all(results())["reliability_diagram"]
        self.assertIn(">0.0<", svg)
        self.assertIn(">1.0<", svg)

    def test_timeline_has_axis_labels(self):
        svg = SvgVisualizationEngine().render_all(results())["surprise_timeline"]
        self.assertIn("Case order", svg)
        self.assertIn("Surprise index", svg)

    def test_confusion_has_percentages_and_totals(self):
        svg = SvgVisualizationEngine().render_all(results())["confusion_heatmap"]
        self.assertIn("25%", svg)
        self.assertIn("Σ", svg)

    def test_observatory_accent_colors_differ(self):
        charts = SvgVisualizationEngine().render_all(results())
        self.assertIn("#4fd1c5", charts["prediction_popularity"])
        self.assertIn("#68d391", charts["reliability_diagram"])
        self.assertIn("#f6ad55", charts["prediction_distribution"])
        self.assertIn("#b794f4", charts["surprise_timeline"])
