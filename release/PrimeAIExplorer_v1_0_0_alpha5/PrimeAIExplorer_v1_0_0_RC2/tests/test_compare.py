...
from primeaiexplorer.compare import build_comparison, markdown_report, html_report, discover_analyses, load_analysis
...
class ComparativeObservatoryTests(unittest.TestCase):
    ...
    def test_legacy_null_metrics_render_in_reports(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            a_path = self._analysis(root, "pilot_a", "Legacy Model", 0.4, 0.2)
            b_path = self._analysis(root, "pilot_b", "Modern Model", 0.6, 0.1)
            summary = json.loads((a_path / "summary.json").read_text(encoding="utf-8"))
            summary["mean_absolute_error"] = None
            summary["mean_signed_error"] = None
            summary["persistence"] = {"switch_rate": None, "max_run_length": None}
            (a_path / "summary.json").write_text(json.dumps(summary), encoding="utf-8")
            comparison = build_comparison([load_analysis(a_path), load_analysis(b_path)])
            markdown = markdown_report(comparison)
            html = html_report(comparison)
            self.assertIn("Legacy Model", markdown)
            self.assertIn("—", markdown)
            self.assertIn("Legacy Model", html)
            self.assertIn("—", html)
