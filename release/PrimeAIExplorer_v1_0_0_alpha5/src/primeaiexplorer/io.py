...
def _iter_standard_aggregate_entries(payload: Any, path: Path) -> Iterator[tuple[str | None, dict[str, Any], str]]:
    ...
    for index, value in enumerate(entries, start=1):
        ...
        response_payload = dict(_extract_response_payload(value, item_source))
        response_payload["_collected_at"] = value.get("collected_at")
        response_payload["_response_seconds"] = value.get("response_seconds") or value.get("duration_seconds") or value.get("latency_seconds")
        yield case_id, response_payload, item_source
...
def load_records(
    ...
    def append_record(
        ...
        collected_at = payload.get("_collected_at")
        raw_seconds = payload.get("_response_seconds")
        response_seconds = None
        if raw_seconds is not None:
            try:
                response_seconds = float(raw_seconds)
        ...
        records.append(
            ResponseRecord(
                ...
                collected_at=collected_at if isinstance(collected_at, str) else None,
