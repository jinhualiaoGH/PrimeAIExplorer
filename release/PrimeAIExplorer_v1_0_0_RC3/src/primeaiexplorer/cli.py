...
from .release import all_required_pass, doctor_checks, release_checks, write_check_report
from .publication import build_publication
from .compare import (
    build_comparison,
    discover_analyses, select_latest_analyses,
    html_report as comparison_html_report,
    load_analysis,
    markdown_report as comparison_markdown_report,
    write_csv as write_comparison_csv,
)
...
def cmd_analyze(args: argparse.Namespace) -> int:
    ...
    print("PrimeAIExplorer v0.7.3 analysis complete")
...
def _refresh_analysis_after_collect(args: argparse.Namespace, pilot: Path, dataset: Path) -> None:
    if not args.refresh_analysis:
        return
    output = Path(args.analysis_output) if args.analysis_output else pilot.parent / "analysis_v070" / pilot.name
...
def _workspace_header(args: argparse.Namespace) -> None:
    ...
    print()
    print("=" * 72)
    print("PrimeAIExplorer v0.7.3 Interactive Workspace")
    print("=" * 72)
...
def _workspace_refresh_namespace(args: argparse.Namespace) -> argparse.Namespace:
    output = args.analysis_output or str(Path(args.responses).parent / "analysis_v070" / Path(args.responses).name)
    return argparse.Namespace(
        responses=args.responses,
        dataset=args.dataset,
        prompts=args.prompts,
...
def _workspace_open_report(args: argparse.Namespace) -> None:
    output = Path(args.analysis_output or (Path(args.responses).parent / "analysis_v070" / Path(args.responses).name))
...
def cmd_compare(args: argparse.Namespace) -> int:
    output = Path(args.output)
    explicit = [Path(value) for value in (args.analysis or [])]
    discovered: list[Path] = []
    if args.experiment_root:
        print("PrimeAIExplorer Comparative Observatory")
        print("=" * 72)
        print(f"Scanning experiment: {Path(args.experiment_root).resolve()}")
        discovered = select_latest_analyses(discover_analyses(Path(args.experiment_root), exclude=output))

    folders: list[Path] = []
    seen: set[Path] = set()
    for folder in [*explicit, *discovered]:
    ...
    if labels and len(labels) != len(explicit):
        raise ValueError("--label must be omitted or supplied once for every explicit --analysis folder")

    valid: list[tuple[Path, str | None]] = []
    missing: list[Path] = []
    for index, folder in enumerate(folders):
        if (folder / "summary.json").is_file() and (folder / "manifest.json").is_file():
            label = labels[index] if index < len(explicit) and labels else None
            valid.append((folder, label))
        else:
            missing.append(folder)

    for folder in missing:
        print(f"[SKIP] Incomplete analysis: {folder}")

    if discovered:
        print(f"Discovered valid analyses: {len(discovered)}")
        for folder in discovered:
            try:
                item = load_analysis(folder)
                print(f"  [FOUND] {item['label']} -> {folder}")
            except Exception:
                print(f"  [FOUND] {folder}")

    if len(valid) < 2:
        print()
        print("Comparison not generated.")
        print("A comparative observatory requires at least two completed analyses.")
        print(f"Completed analyses found: {len(valid)}")
        for folder, label in valid:
            item = load_analysis(folder, label)
            print(f"  [READY] {item['label']} -> {folder}")
        print()
        print("Create another model or pilot analysis, then rerun this command.")
        return 0

    analyses = [load_analysis(folder, label) for folder, label in valid]
    ...
    write_comparison_csv(output / "model_comparison.csv", comparison["comparison_rows"])
    write_comparison_csv(output / "fingerprint_matrix.csv", comparison["fingerprint_matrix"])
    write_comparison_csv(output / "window_comparison.csv", comparison["window_comparison"])
    write_comparison_csv(output / "rankings.csv", comparison["rankings"])
    (output / "report.md").write_text(comparison_markdown_report(comparison), encoding="utf-8")
    (output / "report.html").write_text(comparison_html_report(comparison), encoding="utf-8")
    manifest = {
        "schema_version": "0.7.3",
        "primeaiexplorer_version": __version__,
        "created_utc": datetime.now(timezone.utc).isoformat(),
        "analysis_count": len(analyses),
        "analysis_sources": [item["folder"] for item in analyses],
        "comparison_summary_sha256": sha256_file(summary_path),
        "command": " ".join(sys.argv),
    }
    (output / "manifest.json").write_text(json.dumps(manifest, indent=2) + "\n", encoding="utf-8")
    print("PrimeAIExplorer v0.7.3 comparative analysis complete")
    print(f"  Analyses: {len(analyses)}")
    print(f"  Models:   {len(comparison['models'])}")
    print(f"  Pilots:   {len(comparison['pilots'])}")
    print(f"  Output:   {output.resolve()}")
    for ranking in comparison["rankings"]:
        print(f"  Best {ranking['metric']}: {ranking['best_label']} ({ranking['best_value']:.6g})")
    return 0


def cmd_compare_verify(args: argparse.Namespace) -> int:
    folder = Path(args.comparison)
    required = [
        "comparison_summary.json", "model_comparison.csv", "fingerprint_matrix.csv",
        "window_comparison.csv", "rankings.csv", "manifest.json", "report.md", "report.html",
    ]
    ...
    if manifest.get("comparison_summary_sha256") != sha256_file(folder / "comparison_summary.json"):
        print("[FAIL] comparison_summary.json hash mismatch")
        return 1
    summary = json.loads((folder / "comparison_summary.json").read_text(encoding="utf-8"))
    print("[PASS] Comparative artifact set is complete and internally consistent")
    print(f"[PASS] Analyses: {summary['analysis_count']}")
    print(f"[PASS] Models: {len(summary.get('models', []))}")
    print(f"[PASS] Pilots: {len(summary.get('pilots', []))}")
    return 0
...
def _print_checks(title, checks):
    print(title); print("="*72)
    for c in checks:
        status="PASS" if c.passed else ("WARN" if not c.required else "FAIL")
        print(f"[{status}] {c.name:<36} {c.detail}")
    ok=all_required_pass(checks); print("="*72); print("[PASS] Required checks passed" if ok else "[FAIL] Required checks failed"); return 0 if ok else 1

def cmd_doctor(args):
    checks=doctor_checks(Path(args.directory) if args.directory else None)
    if args.json_output: write_check_report(Path(args.json_output),checks,kind="doctor")
    return _print_checks("PrimeAIExplorer v1.0.0 RC3 Doctor",checks)

def cmd_release_check(args):
    checks=release_checks(Path(args.root))
    if args.json_output: write_check_report(Path(args.json_output),checks,kind="release-check")
    return _print_checks("PrimeAIExplorer v1.0.0 RC3 Release Check",checks)

def cmd_publish(args):
    out=build_publication(Path(args.analysis),Path(args.output))
    print("PrimeAIExplorer v1.0.0 RC3 Publication Export")
    print("="*72)
    print(f"[PASS] Publication package: {out.resolve()}")
    print(f"[PASS] Dashboard: {(out/'dashboard.html').resolve()}")
    print(f"[PASS] Manifest: {(out/'publication_manifest.json').resolve()}")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="paiexp",
        description="PrimeAIExplorer v1.0.0 RC3 scientific AI observatory",
    )
    ...
    compare_parser = sub.add_parser("compare", help="Build an experiment-level comparative observatory from analysis folders")
    compare_parser.add_argument("--analysis", action="append", help="Explicit pilot analysis folder; repeat as needed")
    compare_parser.add_argument("--experiment-root", help="Experiment directory to scan recursively for completed analyses")
    compare_parser.add_argument("--label", action="append", help="Optional display label corresponding to each --analysis")
    compare_parser.add_argument("--output", required=True)
    compare_parser.set_defaults(func=cmd_compare)

    compare_verify_parser = sub.add_parser("compare-verify", help="Verify a comparative observatory artifact set")
    compare_verify_parser.add_argument("--comparison", required=True)
    compare_verify_parser.set_defaults(func=cmd_compare_verify)
    ...
    doctor_parser = sub.add_parser("doctor", help="Check the local PrimeAIExplorer runtime")
    doctor_parser.add_argument("--directory")
    doctor_parser.add_argument("--json-output")
    doctor_parser.set_defaults(func=cmd_doctor)
    release_parser = sub.add_parser("release-check", help="Validate an RC3 release tree")
    release_parser.add_argument("--root", default=".")
    release_parser.add_argument("--json-output")
    release_parser.set_defaults(func=cmd_release_check)
    publish_parser = sub.add_parser("publish", help="Build a portable publication package from a unified analysis")
