...
def cmd_analyze(args: argparse.Namespace) -> int:
    ...
    manifest = {
        ...
        **metadata,
        ...
        "responses_root": str(responses.resolve()),
    ...
    print("PrimeAIExplorer v0.2.1 analysis complete")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="paiexp", description="PrimeAIExplorer v0.2.1 response observatory")
    parser.add_argument("--version", action="version", version=__version__)
    sub = parser.add_subparsers(dest="command", required=True)

    analyze_parser = sub.add_parser("analyze", help="Analyze response JSON files against a canonical cases.csv dataset")
    analyze_parser.add_argument("--responses", required=True)
