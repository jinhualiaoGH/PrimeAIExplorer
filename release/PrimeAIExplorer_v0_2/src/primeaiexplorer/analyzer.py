...
def explanation_profile(explanations: list[str]) -> dict:
    ...
    for text in explanations:
        ...
        if any(k in low for k in ("frequent", "common", "typical", "mode")): categories["frequency_prior"] += 1
        if any(k in low for k in ("random", "uncertain", "unpredict")): categories["uncertainty"] += 1
...
def analyze(records: list[ResponseRecord], bins: int = 10) -> dict:
    ...
    for record in records: by_window[record.window].append(record)
    windows = {}
