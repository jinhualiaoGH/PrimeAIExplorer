...
def markdown_report(summary: dict, metadata: dict) -> str:
    lines = [
        "# PrimeAIExplorer v0.2 Analysis Report", "",
        f"- Experiment: `{metadata['experiment_id']}`",
        f"- Pilot: `{metadata['pilot_id']}`",
        f"- Model: `{metadata['model']}`",
        f"- Responses: **{summary['record_count']}**", "",
        "## Core metrics", "",
        "| Metric | Value |", "|---|---:|",
    ...
    for row in summary["prediction_distribution"]: lines.append(f"| {row['prediction']} | {row['count']} |")
...
def html_report(summary: dict, metadata: dict) -> str:
    ...
    return f'''<!doctype html><html><head><meta charset="utf-8"><title>PrimeAIExplorer v0.2</title>
...
<body><header><h1>PrimeAIExplorer v0.2</h1><p>{html.escape(metadata['experiment_id'])} · {html.escape(metadata['pilot_id'])} · {html.escape(metadata['model'])}</p></header><main>
<div class="cards"><div class="card"><div>Responses</div><div class="big">{summary['record_count']}</div></div><div class="card"><div>Accuracy</div><div class="big">{_pct(summary['accuracy'])}</div></div><div class="card"><div>Brier</div><div class="big">{summary['brier_score']:.4f}</div></div><div class="card"><div>ECE</div><div class="big">{summary['ece']:.4f}</div></div><div class="card"><div>Entropy</div><div class="big">{summary['prediction_entropy_bits']:.3f}</div></div></div>
