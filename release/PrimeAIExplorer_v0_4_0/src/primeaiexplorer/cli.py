...
def cmd_analyze(args: argparse.Namespace) -> int:
    ...
    print("PrimeAIExplorer v0.4.0 analysis complete")
...
def _refresh_analysis_after_collect(args: argparse.Namespace, pilot: Path, dataset: Path) -> None:
    if not args.refresh_analysis:
        return
    output = Path(args.analysis_output) if args.analysis_output else pilot.parent / "analysis_v040" / pilot.name
...
def _workspace_header(args: argparse.Namespace) -> None:
    ...
    print()
    print("=" * 72)
    print("PrimeAIExplorer v0.4.0 Interactive Workspace")
    print("=" * 72)
...
def _workspace_refresh_namespace(args: argparse.Namespace) -> argparse.Namespace:
    output = args.analysis_output or str(Path(args.responses).parent / "analysis_v040" / Path(args.responses).name)
    return argparse.Namespace(
        responses=args.responses,
        dataset=args.dataset,
        prompts=args.prompts,
...
def _workspace_open_report(args: argparse.Namespace) -> None:
    output = Path(args.analysis_output or (Path(args.responses).parent / "analysis_v040" / Path(args.responses).name))
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="paiexp",
        description="PrimeAIExplorer v0.4.0 collection workflow and response observatory",
    )
