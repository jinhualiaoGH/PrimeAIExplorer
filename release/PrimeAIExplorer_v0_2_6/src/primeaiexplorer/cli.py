...
def cmd_analyze(args: argparse.Namespace) -> int:
    ...
    print("PrimeAIExplorer v0.2.6 analysis complete")
...
def _refresh_analysis_after_collect(args: argparse.Namespace, pilot: Path, dataset: Path) -> None:
    if not args.refresh_analysis:
        return
    output = Path(args.analysis_output) if args.analysis_output else pilot.parent / "analysis_v026" / pilot.name
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="paiexp",
        description="PrimeAIExplorer v0.2.6 collection workflow and response observatory",
    )
