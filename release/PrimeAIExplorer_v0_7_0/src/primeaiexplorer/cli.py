...
from .compare import build_comparison, html_report as comparison_html_report, load_analysis, markdown_report as comparison_markdown_report, write_csv as write_comparison_csv
...
def cmd_analyze(args: argparse.Namespace) -> int:
    ...
    print("PrimeAIExplorer v0.7.0 analysis complete")
...
def _workspace_header(args: argparse.Namespace) -> None:
    ...
    print()
    print("=" * 72)
    print("PrimeAIExplorer v0.7.0 Interactive Workspace")
    print("=" * 72)
...
def cmd_compare(args: argparse.Namespace) -> int:
    folders = [Path(value) for value in args.analysis]
    labels = list(args.label or [])
    if labels and len(labels) != len(folders):
        raise ValueError("--label must be omitted or supplied once for every --analysis folder")
    analyses = [
        load_analysis(folder, labels[index] if labels else None)
    ...
    (output / "manifest.json").write_text(json.dumps(manifest, indent=2) + "\n", encoding="utf-8")
    print("PrimeAIExplorer v0.7.0 comparative analysis complete")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="paiexp",
        description="PrimeAIExplorer v0.7.0 comparative AI observatory",
    )
    ...
    compare_parser.add_argument("--analysis", action="append", required=True, help="Pilot analysis folder; repeat at least twice")
