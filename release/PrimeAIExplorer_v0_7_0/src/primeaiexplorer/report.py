...
def html_report(summary, metadata):
    ...
    return f'''<!doctype html><html><head><meta charset="utf-8"><title>PrimeAIExplorer v0.7.0</title><style>
...
</style></head><body><header><h1>PrimeAIExplorer v0.7.0</h1><p>{html.escape(metadata['experiment_id'])} · {html.escape(metadata['pilot_id'])} · {html.escape(metadata['model'])}</p></header><main>
