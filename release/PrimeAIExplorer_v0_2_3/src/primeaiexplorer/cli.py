...
def cmd_analyze(args: argparse.Namespace) -> int:
    ...
    print("PrimeAIExplorer v0.2.3 analysis complete")
...
def cmd_response_check(args: argparse.Namespace) -> int:
    ...
    print(f"[PASS] Collections: {len({record.collection_sha256 for record in records})}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="paiexp",
        description="PrimeAIExplorer v0.2.3 native response observatory",
    )
    ...
    analyze_parser = sub.add_parser(
        "analyze",
        help="Analyze cases.csv with standard, BOM, NDJSON, concatenated, or individual responses",
    )
    ...
    analyze_parser.add_argument(
        "--prompts",
        help="Optional pilot prompt directory used to assign missing case IDs; auto-discovered when omitted",
    )
    ...
    response_parser = sub.add_parser(
        "response-check",
        help="Validate standard, BOM, NDJSON, concatenated, aggregate, or individual responses",
    )
