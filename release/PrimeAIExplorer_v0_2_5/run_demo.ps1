...
$Output = Join-Path $Demo "analysis_v025"

Write-Host ""
Write-Host "PrimeAIExplorer v0.2.5 - Collection Assistant Demo"
Write-Host ("=" * 72)
...
& $Cli collect --responses $Pilot --dataset $Dataset --model "GPT-5.6 Thinking"
& $Cli response-check --responses $Pilot --dataset $Dataset
& $Cli analyze --responses $Pilot --dataset $Dataset --output $Output --model "GPT-5.6-Thinking" --experiment-id "EXP-000001" --pilot-id "pilot_002"
